...
namespace GUI
{
    public partial class frmHoaDonBanSach : Form
    {
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            HoaDon_BUS.XoaHoaDonByMa(int.Parse(txtMaHoaDon.Text));
            MessageBox.Show("Xoá hóa đơn thành công", "Thông báo", MessageBoxButtons.OK);
            foreach(DataGridViewCell item in dgvHoaDon.SelectedCells)
            {
                if (item.Selected)
                {
                    dgvHoaDon.Rows.RemoveAt(item.RowIndex);
                }
            }
        }
    }
